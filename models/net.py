...
from models.Refine import RefineNet
...
class Effi_MVS(nn.Module):
    def __init__(self, args, depth_interals_ratio=[4,2,1], stage_channel=True):
        ...
        self.conv_re = ConvBnReLU(in_channels=6, out_channels=3)
        self.conv1 = ConvBnReLU(3, 16)
        self.conv2 = ConvBnReLU(1, 16)
        ...
        for stage_idx in range(self.num_stage):
            ...
            if stage_idx == self.num_stage - 1:
                ...
                for i in range(view_num):
                    ...
                    warped_image = disp_warp(imgs[:, i + 1], final_depth.unsqueeze(1))[0]
                    # print('wa',warped_image.shape)
                    # print('img', imgs[:, 0].shape)
                    error = warped_image - imgs[:, 0]
                    reprojection_error = reprojection_error + error
                re = torch.cat((reprojection_error, imgs[:, 0]), dim=1)
                re = self.conv_re(re)

                final_depth = self.refine_network(re, final_depth.unsqueeze(1)).squeeze(1)  # CasMVSNet的refine
